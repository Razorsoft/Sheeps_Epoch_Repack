...
if (spectate) then {{if (_x != player) then {nlist set [count nlist, name _x];};} forEach playableUnits;	
	shnmenu = {_pmenu = [["",true],[_n2c, [-1], "", -5, [["expression", ""]], "1", "0"]];
	...
	while {selecteditem==""} do{
		...
		shnext = false;};
if (selecteditem!= "exitscript") then{_name = selecteditem;{if(format[name _x] == _name) then {	
	...
	spectate = false;};
if (!spectate) then {titleText ["Back to player...","PLAIN DOWN"];titleFadeOut 4;};