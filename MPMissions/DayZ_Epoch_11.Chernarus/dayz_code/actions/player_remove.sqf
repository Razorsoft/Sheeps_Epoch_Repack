...
for "_i" from 0 to ((count allbuildables) - 1) do
	...
	if (_qtyTankTrap > 0) then {
		for "_i" from 1 to _qtyTankTrap do { _result = [player,"ItemTankTrap"] call BIS_fnc_invAdd;  };
	};
	if (_qtyWire > 0) then {
		for "_i" from 1 to _qtyWire do { _result = [player,"ItemWire"] call BIS_fnc_invAdd;  };
	};
	if (_qtySandbag > 0) then {
		for "_i" from 1 to _qtySandbag do { _result = [player,"ItemSandbag"] call BIS_fnc_invAdd;  };
	};
	if (_qtyLumber > 0) then {
		for "_i" from 1 to _qtyLumber do { _result = [player,"PartWoodLumber"] call BIS_fnc_invAdd; };
	};
	if (_qtyPlywood > 0) then {
		for "_i" from 1 to _qtyPlywood do { _result = [player,"PartWoodPlywood"] call BIS_fnc_invAdd;  };
	};
	if (_qtyGrenade > 0) then {
		for "_i" from 1 to _qtyGrenade do { _result = [player,"HandGrenade_West"] call BIS_fnc_invAdd;  };
	};
	if (_qtyHesco > 0) then {
		for "_i" from 1 to _qtyHesco do { _result = [player,"ItemSandbagLarge"] call BIS_fnc_invAdd;  };
	};
	if (_qtySandbagNest > 0) then {
		for "_i" from 1 to _qtySandbagNest do { _result = [player,"sandbag_nest_kit"] call BIS_fnc_invAdd;  };
	};
	if (_qtyScrap > 0) then {
		for "_i" from 1 to _qtyScrap do { _result = [player,"PartGeneric"] call BIS_fnc_invAdd;  };
	};
	if (_qtyPole > 0) then {
		for "_i" from 1 to _qtyPole do { _result = [player,"ItemPole"] call BIS_fnc_invAdd;  };
	};
	if (_qtyCorrugatedFence > 0) then {
		for "_i" from 1 to _qtyCorrugatedFence do { _result = [player,"ItemCorrugated"] call BIS_fnc_invAdd;  };
	};
	if (_qtyStorageShed > 0) then {
		for "_i" from 1 to _qtyStorageShed do { _result = [player,"storage_shed_kit"] call BIS_fnc_invAdd;  };
	};
	if (_qtyDesertCamoNet > 0) then {
		for "_i" from 1 to _qtyDesertCamoNet do { _result = [player,"desert_net_kit"] call BIS_fnc_invAdd;  };
	};
	if (_qtyForestCamoNet > 0) then {
		for "_i" from 1 to _qtyForestCamoNet do { _result = [player,"forest_net_kit"] call BIS_fnc_invAdd;  };
	};
	if (_qtyWoodShed > 0) then {
		for "_i" from 1 to _qtyWoodShed do { _result = [player,"wooden_shed_kit"] call BIS_fnc_invAdd;  };
	};
	if (_qtyBurlap > 0) then {
		for "_i" from 1 to _qtyBurlap do { _result = [player,"ItemBurlap"] call BIS_fnc_invAdd;  };
	};
	if (_qtyGlass > 0) then {
		for "_i" from 1 to _qtyGlass do { _result = [player,"PartGlass"] call BIS_fnc_invAdd;  };
	};
...
if ((getPlayerUID player) in ["010101"]) then {
	call _func_ownerRemove;
};
